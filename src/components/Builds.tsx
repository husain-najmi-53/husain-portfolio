...
function BuildCard({ build, index }: { build: Build; index: number }) {
  return (
    <Reveal
      delay={0.05 * index}
      className={build.flagship ? "md:col-span-2" : ""}
    >
      <div className="lift bg-white rounded-2xl shadow-sm p-7 md:p-9 h-full flex flex-col">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {build.stack.map((s) => (
            <span
              key={s}
              className="text-xs px-2.5 py-1 rounded-full bg-ink/5 text-ink-soft"
            >
              {s}
            </span>
          ))}
          ...
          <div className="flex flex-wrap gap-3">
            {build.links.map((link) => (
              <a
                key={link.href}
                href={
                  link.href.startsWith("http") ? link.href : withBase(link.href)
                }
                target={link.href.startsWith("http") ? "_blank" : undefined}
                rel="noreferrer"
                download={
                  link.label.toLowerCase().includes("download") || undefined
                }
